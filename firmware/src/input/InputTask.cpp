...
// Read the cached PCF byte populated by pcfPollerTask. Single-byte access
// on Xtensa is atomic; no mutex needed. Apply mask + plausibility filter
// AND a freshness gate — if the cache is too old (poller stalled on Wire),
// return false so the caller treats the input state as "unknown" and
// freezes its debounce state machine.
bool readPcf(uint8_t* out) {
    uint32_t age = millis() - g_latestPcfReadMs;
    if (age > PCF_STALE_THRESH_MS) return false;   // poller stuck → freeze
